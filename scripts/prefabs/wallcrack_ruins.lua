...
local function fn()
  ...
  inst.components.workable:SetOnFinishCallback(function() reveal() end)

  inst:ListenForEvent("death", function(_) reveal() end)
  inst:ListenForEvent("endquake", function() reveal() end, GetWorld())

  inst:ListenForEvent("exitlimbo", function(_)
    -- Self destruct if this door has already been unlocked
    if inst.door and not inst.door:HasTag("secret") then reveal() end
  end)
