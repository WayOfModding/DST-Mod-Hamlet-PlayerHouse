...
local VALID_TILES = {GROUND.SUBURB,GROUND.FOUNDATION}
            ...
            if WorldSim:GetTile(newpt.x, newpt.z) > 1 then                
                if math.random() < 0.15 or (t<math.abs(4) and i< math.abs(4) ) then
                    if testTile(newpt,VALID_TILES)  then
                        ...
                        placeTile(newpt,GROUND.FOUNDATION)
                    end
                end
            end
        end
...
local function setParkCoord(pt,dir,i,offset, city)
    ...
    for x=-2,2,1 do
        for y=-2,2,1 do
            ...
            if not ground or ground ~= GROUND.FOUNDATION then
                pass = false
                break
            end
        end
...
local function makeroad(pt,dir,suburb, city)
    ...
    if step == stepMax then
        ...
        if math.random() < 0.9 then -- only continue from here some of the time
            table.insert(OPEN_NODES,{pt=newpt,dir=dirset})   
        end
    end
end
...
local function createcity(startNode,city)
    ...
    -- sets the possible directins from this node to all 4 
    table.insert(OPEN_NODES, {pt=start,dir={true,true,true,true}})

    -- randomly shrink the number of paths from a suburb road spawn node
    if startNode.suburb then
       OPEN_NODES[1].dir[math.random(1,4)] = false
       OPEN_NODES[1].dir[math.random(1,4)] = false
       OPEN_NODES[1].dir[math.random(1,4)] = false
    end
    
    if testTile(start) then
        placeTileCity(start)
    end

    while #OPEN_NODES > 0 do
        for dir,shouldgo in ipairs(OPEN_NODES[1].dir) do
            if shouldgo then
                makeroad(OPEN_NODES[1].pt,dir, startNode.suburb, city )
            end
        end
        table.insert(CLOSED_NODES,OPEN_NODES[1])        
        table.remove(OPEN_NODES,1)
    end
end
...
--[[
    ...
    local placed_farms = 0 
    local breaklimit = 0
    while TOTALFARMS > placed_farms and breaklimit < 50 do
        local testedNodes = {}
        local totalNodes = #nodes
        local finished = false
        ...
        while #testedNodes < totalNodes and finished == false do
            ...
            if untested then
                ...
                if place_farm then
                    local choice = FARM_CHOICES[math.random(1,#FARM_CHOICES)]
    ...
    for i,node in ipairs(nodes)do       
        ...
        if #prefabs == 0 then
            if testTile({x=node.cent[1], z=node.cent[2]},{GROUND.FIELDS}) then
...
--[[
                    ...
                    local nodedata = {cent={c_x, c_y}}

                    if inInNestedList(node, topology_save.GlobalTags["Suburb"]) then                    
                        nodedata.suburb = true
                    end                

                    if inInNestedList(node, topology_save.GlobalTags["City_Foundation"]) then                    
    ...
    for city_ID,city in ipairs(cities)do    
        for t,citynode  in ipairs(city.citynodes)do
            createcity(citynode,city)
        end
